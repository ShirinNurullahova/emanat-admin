...
import convertDataToHtml from "../../../editor/parser/custom-parser";
...
export const Card = ({ setId, setBtn, btn }) => {
  ...
  const fetchData = () => {
    axios
      .get(`${process.env.REACT_APP_URL}/admin/news`, {
        headers: {
          "accept-language": "az",
        },
      })
  ...
  const handleDeleteCard = async (id) => {
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_URL}/admin/news/${id}`
      );
      setBtn(null);
              ...
              <div onClick={() => onClickHandler(e.id)}>
                ...
                <div
                  dangerouslySetInnerHTML={{
                    __html: convertDataToHtml(JSON.parse(e.title).blocks),
                  }}
                ></div>
