...
const JoinUs = () => {
    ...
    const fetchData = () => {
        axios.get((`${process.env.REACT_APP_URL}/admin/main/team`))
            .then(res => {
                ...
                initialValuesRaw.MainJoinTeamImage = res.data[0]?.images[0]?.url;
    ...
    const onSubmitHandler = async (values) => {
        ...
        if (values.image) {
            dataForm.append('MainJoinTeamImage', values.image)
        } else {
            dataForm.append('MainJoinTeamImage', values.MainJoinTeamImage)
        }
     
        try {
            const response = await axios.patch(`${process.env.REACT_APP_URL}/admin/main/team`, dataForm)
